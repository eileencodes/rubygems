...
RSpec.describe Bundler::LockfileParser do
  ...
  describe "#initialize" do
    ...
    context "when a GEM source has binary remotes" do
      let(:lockfile_contents) { <<~L }
        GEM
          remote: https://rubygems.org/
      ...
      it "parses the binary remote" do
        gem_source = subject.sources.find {|s| s.is_a?(Bundler::Source::Rubygems) }
        expect(gem_source.binary_remotes).to eq [Gem::URI("https://build-farm.example.com/")]
      end
    ...
    context "when a GEM source has multiple binary remotes" do
      let(:lockfile_contents) { <<~L }
        GEM
          remote: https://rubygems.org/
          binary: https://first.example.com/
          binary: https://second.example.com/
          specs:
            rake (10.3.2)
      ...
      it "parses multiple binary remotes" do
        gem_source = subject.sources.find {|s| s.is_a?(Bundler::Source::Rubygems) }
        expect(gem_source.binary_remotes.size).to eq 2
      end
    end
